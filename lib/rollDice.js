...
// gets damage done
function damageRoll(damage_length) {
    // for each damage type within the action...
    for (let i = 0; i < damage_length; i++) {
        // ... calculate damage
        console.log(this.actions[1].damage[i].damage_dice);
...
// // calculate to see if the monster's attack succeeds
// const getRandomNumber = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
// const rollD20 = () => getRandomNumber(1, 20);
